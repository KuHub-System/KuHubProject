...
const InventarioPage: React.FC = () => {
  ...
  // ── Estado del tab "Movimientos" (fusionado desde movimientos-producto.tsx) ──
  const initialMovNombre = React.useMemo(
  ...
  const [movNombreProducto, setMovNombreProducto] = React.useState(initialMovNombre);
  ...
  }, [activeTab, movNombreProducto, movNombreResponsable, movTipoMovimiento, movOrden, movDateRangeValue]);
  ...
  }, [cargarMasMovimientos, activeTab]);

  // Carga inicial de movimientos: al entrar directo por /movimientos o al abrir la pestaña por primera vez
  const movYaCargadoRef = React.useRef(false);
  React.useEffect(() => {
    if (activeTab !== 'movimientos' || movYaCargadoRef.current) return;
    movYaCargadoRef.current = true;
    setMovDebouncedRequest({
      page: 1,
      nombreProducto: initialMovNombre,
      nombreResponsable: '',
